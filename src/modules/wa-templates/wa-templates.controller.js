...
import * as service from './wa-templates.service.js';
...
export const remove = asyncHandler(async (req, res) => {
  if (req.user.role !== 'owner') throw new AppError('Solo owners pueden eliminar templates', 403);
  await service.remove(req.tenantId, req.params.id);
  res.status(204).send();
});

export const syncFromMeta = asyncHandler(async (req, res) => {
  const { wabaId } = req.body;
  res.json(await service.syncFromMeta(req.tenantId, wabaId));
});
